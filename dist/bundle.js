...
module.exports = __webpack_require__(/*! /home/dara/phaser-es6-webpack/src/main.js */328);
...
var _Boot = __webpack_require__(/*! ./states/Boot */ 329);

var _Boot2 = _interopRequireDefault(_Boot);

var _Preload = __webpack_require__(/*! ./states/Preload */ 330);

var _Preload2 = _interopRequireDefault(_Preload);

var _GameTitle = __webpack_require__(/*! ./states/GameTitle */ 331);

var _GameTitle2 = _interopRequireDefault(_GameTitle);

var _Main = __webpack_require__(/*! ./states/Main */ 332);
...
var Game = function (_Phaser$Game) {
	...
	function Game() {
		...
		var _this = _possibleConstructorReturn(this, (Game.__proto__ || Object.getPrototypeOf(Game)).call(this, window.innerWidth * window.devicePixelRatio, window.innerHeight * window.devicePixelRatio, Phaser.CANVAS));
...
var _Background = __webpack_require__(/*! ../objects/Background */ 333);

var _Background2 = _interopRequireDefault(_Background);

var _Player = __webpack_require__(/*! ../objects/Player */ 336);

var _Player2 = _interopRequireDefault(_Player);

var _Controls = __webpack_require__(/*! ../objects/Controls */ 337);

var _Controls2 = _interopRequireDefault(_Controls);

var _Level = __webpack_require__(/*! ../objects/Level1 */ 338);
...
var Main = function (_Phaser$State) {
  ...
  _createClass(Main, [{
    ...
    value: function create() {
      ...
      var levelWidth = this.game.width * 5;

      this.background = new _Background2.default(this.game, levelWidth);

      this.game.world.setBounds(0, 0, levelWidth, this.game.height);

      this.controls = new _Controls2.default(this.game);
      this.player = new _Player2.default(this.game, this.controls);

      this.controls.setPlayerSprite(this.player.getPlayerSprite());

      this.level1 = new _Level2.default(this.game, this.player, levelWidth);
...
var _Weather = __webpack_require__(/*! ./Weather */ 334);

var _Weather2 = _interopRequireDefault(_Weather);

var _DayCycle = __webpack_require__(/*! ./DayCycle */ 335);
...
var Background = function () {
  function Background(game, levelWidth) {
    _classCallCheck(this, Background);

    this.game = game;
    this.levelWidth = levelWidth;

    this.dayCycle = new _DayCycle2.default(this.game, 1000 * 60 * 1);
    ...
    this.fog1 = this.game.add.tileSprite(0, 0, levelWidth, this.game.height / this.densityFactor, 'fog1');
    this.backgroundGroup.add(this.fog1);

    this.fog2 = this.game.add.tileSprite(0, 0, levelWidth, this.game.height / this.densityFactor, 'fog2');
    ...
    this.stars = this.game.add.tileSprite(0, 0, levelWidth, this.game.height, 'stars');
    this.backgroundGroup.add(this.stars);

    this.mountains1 = this.game.add.tileSprite(0, this.game.height - this.game.cache.getImage('mountains-back').height * this.densityFactor, levelWidth, this.game.cache.getImage('mountains-back').height, 'mountains-back');
    this.backgroundGroup.add(this.mountains1);

    this.mountains2 = this.game.add.tileSprite(0, this.game.height - this.game.cache.getImage('mountains-mid1').height * this.densityFactor, levelWidth, this.game.cache.getImage('mountains-mid1').height, 'mountains-mid1');
    this.backgroundGroup.add(this.mountains2);

    this.fog3 = this.game.add.tileSprite(0, 0, levelWidth, this.game.height / this.densityFactor, 'fog3');
    ...
    this.mountains3 = this.game.add.tileSprite(0, this.game.height - this.game.cache.getImage('mountains-mid2').height * this.densityFactor, levelWidth, this.game.cache.getImage('mountains-mid2').height, 'mountains-mid2');
    this.backgroundGroup.add(this.mountains3);

    this.fog4 = this.game.add.tileSprite(0, 0, levelWidth, this.game.height / this.densityFactor, 'fog4');
    this.backgroundGroup.add(this.fog4);

    this.backgroundSprite = this.game.add.tileSprite(0, 0, levelWidth, this.game.height, 'backgroundSprite');
    ...
    { sprite: this.backgroundSprite, less: true }, { sprite: this.stars, stars: true }];
...
var Weather = function () {
  ...
  _createClass(Weather, [{
    ...
    value: function addRain() {
      ...
      setTimeout(function () {
        _this.rain.volume = 0.5;
        _this.rain.fadeIn(2500, true);

        var rainParticle = _this.game.add.bitmapData(15, 50);

        rainParticle.ctx.rect(0, 0, 15, 1000);
        rainParticle.ctx.fillStyle = '#7d7d7d';
        rainParticle.ctx.fill();

        _this.emitter = _this.game.add.emitter(_this.game.world.centerX, -100, 400);

        _this.emitter.width = _this.game.world.width;
        _this.emitter.angle = 4;

        _this.emitter.makeParticles(rainParticle);

        _this.emitter.minParticleScale = 0.3;
        _this.emitter.maxParticleScale = 0.6;

        _this.emitter.setYSpeed(1200, 2000);
        _this.emitter.setXSpeed(0, 5);

        _this.emitter.minRotation = 0;
        _this.emitter.maxRotation = 1;

        _this.emitter.start(false, 1600, 5, 0);

        var rainParticle2 = _this.game.add.bitmapData(15, 50);

        rainParticle2.ctx.rect(0, 0, 15, 1000);
        rainParticle2.ctx.fillStyle = '#bbb';
        rainParticle2.ctx.fill();

        _this.emitter2 = _this.game.add.emitter(_this.game.world.centerX, -100, 400);

        _this.emitter2.width = _this.game.world.width;
        _this.emitter2.angle = 4;

        _this.emitter2.makeParticles(rainParticle2);

        _this.emitter2.minParticleScale = 0.3;
        _this.emitter2.maxParticleScale = 0.6;

        _this.emitter2.setYSpeed(1200, 2000);
        _this.emitter2.setXSpeed(0, 5);

        _this.emitter2.minRotation = 0;
        _this.emitter2.maxRotation = 1;

        _this.emitter2.start(false, 1600, 5, 0);

        _this.emitterGroup = _this.game.add.group();
        _this.emitterGroup.add(_this.emitter);
        _this.emitterGroup.add(_this.emitter2);

        _this.emitterGroup.fixedToCamera = true;
      }, 2000);
    }
  }, {
    key: 'removeRain',
...
var DayCycle = function () {
  ...
  _createClass(DayCycle, [{
    ...
    value: function tweenTint(spriteToTween, darker) {
      ...
      var colorBlend = { alpha: darker ? 0 : 1 };
      // alert();

      this.game.add.tween(colorBlend).to({ alpha: darker ? 1 : 0 }, this.dayLength, Phaser.Easing.Default, false).onUpdateCallback(function () {
        if (spriteToTween.less !== true) spriteToTween.sprite.alpha = colorBlend.alpha;else spriteToTween.sprite.alpha = colorBlend.alpha / 2;
        ...
        var val = 1 - Math.max(colorBlend.alpha, 0.3);

        var hexVal = Math.floor(val * 255).toString(16);

        if (hexVal.length < 2) {
          hexVal = '0' + hexVal;
        }

        _this3.game.stage.backgroundColor = '#' + hexVal + '' + hexVal + '' + hexVal;
        // console.log(hexVal)
      }).start();
    }
  }]);
...
var Player = function () {
  ...
  _createClass(Player, [{
    ...
    key: 'update',
    value: function update() {
      var controls = this.controls.getOutputs();
    ...
    key: 'getPlayerSprite',
    value: function getPlayerSprite() {
      return this.player;
    }
  }]);
...
var Controls = function () {
  function Controls(game) {
    ...
    this.left = this.game.add.tileSprite(0, 0, this.game.width / 3, this.game.height, 'left');

    this.right = this.game.add.tileSprite(2 * this.game.width / 3, 0, this.game.width / 3, this.game.height, 'right');

    this.jump1 = this.game.add.tileSprite(0, 0, this.game.width / 3, this.game.height / 3, 'left');

    this.jump2 = this.game.add.tileSprite(2 * this.game.width / 3, 0, this.game.width / 3, this.game.height / 3, 'jump');

    this.weather = this.game.add.tileSprite(this.game.width / 3, 0, this.game.width / 3, this.game.height / 4, 'weather');
    ...
    this.left.events.onInputDown.add(function () {
      return _this.turnOn(['left']);
    });
    this.right.events.onInputDown.add(function () {
      return _this.turnOn(['right']);
    });

    this.jump1v;
    this.jump2.events.onInputOver.add(function () {
      return _this.turnOn(['jump', 'right']);
    });

    this.jump1.events.onInputDown.add(function () {
      return _this.turnOn(['jump']);
    });
    this.jump2.events.onInputDown.add(function () {
      return _this.turnOn(['jump']);
    });
    ...
    this.weather.events.onInputDown.add(function () {
      return _this.turnOn(['weather']);
    });

    this.left.events.onInputUp.add(function () {
      return _this.turnOff(['jump', 'left']);
    });
    this.right.events.onInputUp.add(function () {
      return _this.turnOff(['jump', 'right']);
    });

    this.jump1.events.onInputUp.add(function () {
      return _this.turnOff(['jump', 'left']);
    });
    this.jump2.events.onInputUp.add(function () {
      return _this.turnOff(['jump', 'right']);
    });
    this.jump1.events.onInputOut.add(function () {
      return _this.turnOff(['jump']);
    });
    this.jump2.events.onInputOut.add(function () {
      return _this.turnOff(['jump']);
    });
  ...
  _createClass(Controls, [{
    key: 'setPlayerSprite',
    value: function setPlayerSprite(player) {
      this.player = player;
    }
  }, {
    key: 'turnOn',
    value: function turnOn(key) {
    ...
    key: 'turnOff',
    value: function turnOff(key) {
      var _this2 = this;

      if (key[0] === 'jump') {
        this.jump1.events.onInputOver.removeAll();
        this.jump2.events.onInputOver.removeAll();
        this.jump1.events.onInputOver.add(function () {
          return _this2.turnOn(['jump', 'left']);
        });
        this.jump2.events.onInputOver.add(function () {
          return _this2.turnOn(['jump', 'right']);
        });

        this.jump1.events.onInputOut.removeAll();
        this.jump2.events.onInputOut.removeAll();
        this.jump1.events.onInputOut.add(function () {
          return _this2.turnOff(['jump']);
        });
        this.jump2.events.onInputOut.add(function () {
          return _this2.turnOff(['jump']);
        });
...
var _Fire = __webpack_require__(/*! ./Fire */ 339);

var _Fire2 = _interopRequireDefault(_Fire);

var _TextOverlay = __webpack_require__(/*! ./TextOverlay */ 340);
...
var Level1 = function () {
  function Level1(game, player, levelWidth) {
    ...
    this.grass = game.add.tileSprite(0, game.height - game.cache.getImage('grass').height * this.densityFactor, levelWidth / this.densityFactor, game.cache.getImage('grass').height, 'grass');
    ...
    this.block = game.add.sprite(game.width / 2, game.height - this.grass.body.height - game.cache.getImage('grass').height * this.densityFactor + 20, 'block');
    ...
    this.fire = new _Fire2.default(game, levelWidth - 500, game.height - 20);

    game.world.bringToTop(this.grass);

    this.titleText = new _TextOverlay2.default(game, 'Fog', 150, true);

    this.text1 = new _TextOverlay2.default(game, 'It\'s pretty scary,', game.width + game.width / 2);
    this.text2 = new _TextOverlay2.default(game, 'but go in you must,', game.width * 2 + game.width / 2);
    this.text3 = new _TextOverlay2.default(game, 'these tangerine fairies,', game.width * 3 + game.width / 2);
    this.text4 = new _TextOverlay2.default(game, 'will turn you to dust.', game.width * 4 + game.width / 2);
  ...
  _createClass(Level1, [{
    ...
    key: 'update',
    value: function update() {
      var distanceFromEnd = this.levelWidth - this.player.getPlayerSprite().x;
...
var Fire = function Fire(game, posX, posY) {
  ...
  var grd2 = bmpd2.ctx.createRadialGradient(pSize / 2, pSize / 2, 2, pSize / 2, pSize / 2, pSize * 0.5);
  ...
  bmpd2.ctx.fill();
  var smokeEmitter = game.add.emitter(posX, posY - 20, 100);
  ...
  // on the outside. Use it to draw a circle that will be used
  // by the FireParticle class.
  var grd = bmpd.ctx.createRadialGradient(pSize / 2, pSize / 2, 2, pSize / 2, pSize / 2, pSize * 0.5);
  ...
  var fireEmitter = game.add.emitter(posX, posY - 20, 100);
...
var TextOverlay = function () {
  function TextOverlay(game, text, position, title) {
  ...
  _createClass(TextOverlay, [{
    ...
    value: function update(playerPosition) {
      var distance = Math.abs(playerPosition - this.text.x);
